...
class _CameraDetailPageState extends State<CameraDetailPage>
  ...
  Future<void> _loadCameraDetails() async {
    ...
    try {
      final response = await _apiService.get('/cameras/${_camera.id}');
      
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        setState(() {
          _camera = Camera.fromJson(data);
        });
        _addDebugLog('Camera details loaded successfully');
      } else {
        throw Exception('Failed to load camera details: ${response.statusCode}');
      }
    } catch (e) {
  ...
  Future<void> _loadCameraActivities() async {
    ...
    try {
      final response = await _apiService.get('/cameras/${_camera.id}/activities');
      
      if (response.statusCode == 200) {
        final List<dynamic> data = json.decode(response.body);
        setState(() {
          _activities = data.map((json) => CameraActivity.fromJson(json)).toList();
        });
  ...
  Future<void> _loadCameraSettings() async {
    ...
    try {
      final response = await _apiService.get('/cameras/${_camera.id}/settings');
  ...
  Future<void> _checkCameraStatus() async {
    ...
    try {
      final response = await _apiService.get('/cameras/${_camera.id}/status');
  ...
  Future<void> _toggleRecording() async {
    ...
    try {
      final response = await _apiService.post(
        '/cameras/${_camera.id}/toggle-recording',
        body: {},
      );
